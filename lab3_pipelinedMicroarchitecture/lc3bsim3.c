...
/*                                                             */
/***************************************************************/
void dcache_access(int dcache_addr, int *read_word, int write_word, int *dCacheR,
        int WE0, int WE1) {
  ...
  if (!random) {
    ...
    *read_word = (MEMORY[addr][1] << 8) | (MEMORY[addr][0] & 0x00FF);
    if(WE0) MEMORY[addr][0] = write_word & 0x00FF;
    if(WE1) MEMORY[addr][1] = (write_word & 0xFF00) >> 8;
  }
}
/***************************************************************/
/*                                                             */
...
/* MEM Signals */
int memTrapPC, memTargetPC, memPCMux, v_mem_ld_reg, v_mem_ld_cc, mem_reg_id;
/************************* MEM_stage() *************************/
void MEM_stage() {

  int write_word, WE0, WE1, dCacheR, sr_data;
  int read_word = 0;
  int dataSize = Get_DATA_SIZE(PS.MEM_CS);
  int dCacheEN = Get_DCACHE_EN(PS.MEM_CS);
  int dCacheRW = Get_DCACHE_RW(PS.MEM_CS);
  int M0 = get_bits(PS.MEM_ADDRESS, 0, 0);

  memTargetPC  = PS.MEM_ADDRESS;
  ...
  if ((dCacheEN)) {
    dcache_access(PS.MEM_ADDRESS, &read_word, write_word, &dCacheR, WE0, WE1);
  }
  ...
  if (dataSize) {
    ...
    if (M0) {
      read_word = Low16bits(SEXT((get_bits(read_word, 15, 8)), 8));
    } else {
      read_word = Low16bits(SEXT(get_bits(read_word, 7, 0), 8));
    }
    memTrapPC = read_word;
  }
  ...
  int memBR = PS.MEM_CC & (get_bits(PS.MEM_IR, 11, 9));
...
void AGEX_stage() {
  ...
  int addr2Mux, addr1Mux, addrMux, sr2, ALU, SHF, aluResult, agexldMEM;
  int agexADDR1MUX = Get_ADDR1MUX(PS.AGEX_CS);
  int agexADDR2MUX = Get_ADDR2MUX(PS.AGEX_CS);
  int agexLSHF1 = Get_LSHF1(PS.AGEX_CS);
  int agexADDRESSMUX = Get_ADDRESSMUX(PS.AGEX_CS);
  int agexSR2MUX = Get_SR2MUX(PS.AGEX_CS);
  int agexALUK = Get_ALUK(PS.AGEX_CS);
  int agexALURESULTMUX = Get_ALU_RESULTMUX(PS.AGEX_CS);
  ...
  switch (agexADDR2MUX) {
    case 0:
      ...
      break;
    case 1:
      addr2Mux = SEXT(get_bits(PS.AGEX_IR, 5, 0), 6);
      break;
    case 2:
      addr2Mux = SEXT(get_bits(PS.AGEX_IR, 8, 0), 9);
      break;
    case 3:
      addr2Mux = SEXT(get_bits(PS.AGEX_IR, 10, 0), 11);
      break;
    default: addr2Mux = 0x0000;
      break;
  }
  ...
  if (agexLSHF1) {
    addr2Mux = LSHF(addr2Mux, 1);
  }

  if (agexADDRESSMUX) {
    addrMux = Low16bits(addr1Mux + addr2Mux);
  } else {
    addrMux = LSHF(ZEXT(get_bits(PS.AGEX_IR, 7, 0), 8), 1);
  }
  ...
  switch (get_bits(PS.AGEX_IR, 5, 4)) {
    case 0:
      SHF = LSHF(PS.AGEX_SR1, get_bits(PS.AGEX_IR, 3, 0));
      break;
    case 1:
      SHF = RSHF(PS.AGEX_SR1, get_bits(PS.AGEX_IR, 3, 0), 0);
      break;
    case 2:
      SHF = LSHF(PS.AGEX_SR1, get_bits(PS.AGEX_IR, 3, 0));
      break;
    case 3:
      SHF = RSHF(PS.AGEX_SR1, get_bits(PS.AGEX_IR, 3, 0), (get_bits(PS.AGEX_SR1, 15, 15)));
      break;
    default:
      break;
  }
  ...
  int brSTALL = Get_AGEX_BR_STALL(PS.AGEX_CS);
  int ldREG = Get_AGEX_LD_REG(PS.AGEX_CS);
  int ldCC = Get_AGEX_LD_CC(PS.AGEX_CS);
...
void DE_stage() {
  ...
  int deADDRESS = get_bits(PS.DE_IR,15,11) << 1 | get_bits(PS.DE_IR,5,5);
  int SR1pos, SR1val, SR2pos, SR2val, drid, ldAGEX;
  int deSR2 = get_bits(PS.DE_IR, 13, 13);
  int deBRSTALL = Get_DE_BR_STALL(CONTROL_STORE[deADDRESS]);
  int deBROP = Get_DE_BR_OP(CONTROL_STORE[deADDRESS]);
  int drMUX = Get_DRMUX(CONTROL_STORE[deADDRESS]);
  int deSR1NEEDED = Get_SR1_NEEDED(CONTROL_STORE[deADDRESS]);
  int deSR2NEEDED = Get_SR2_NEEDED(CONTROL_STORE[deADDRESS]);

  SR1pos = get_bits(PS.DE_IR, 8, 6);
  SR1val = REGS[SR1pos];

  if (deSR2) {
      SR2pos = get_bits(PS.DE_IR, 11, 9);
  } else {
      SR2pos = get_bits(PS.DE_IR, 2, 0);
  }
  SR2val = REGS[SR2pos];

  if (PS.DE_V & deBRSTALL) {
    v_de_br_stall = 1;
  } else {
    v_de_br_stall = 0;
  }
  ...
  int agexSTALL1, memSTALL1, srSTALL1, agexSTALL2, memSTALL2, srSTALL2;
  /* Dependency Check (DEPSTALL) */
  if (PS.DE_V) {
    mem_reg_id = PS.MEM_DRID;
    agexSTALL1 = ((SR1pos == agex_reg_id) & v_agex_ld_reg);
    memSTALL1 = ((SR1pos == mem_reg_id) & v_mem_ld_reg);
    srSTALL1 = ((SR1pos == sr_reg_id) & v_sr_ld_reg);
    agexSTALL2 = ((SR2pos == agex_reg_id) & v_agex_ld_reg);
    memSTALL2 = ((SR2pos == mem_reg_id) & v_mem_ld_reg);
    srSTALL2 = ((SR2pos == sr_reg_id) & v_sr_ld_reg);
    dep_stall = 0;
    if (deBROP) {
    ...
    if (deSR1NEEDED) {
      if (agexSTALL1 | memSTALL1 | srSTALL1) {
        dep_stall = 1;
      }
    }
    if (deSR2NEEDED) {
      if (agexSTALL2 | memSTALL2 | srSTALL2) {
        dep_stall = 1;
      }
    }
  } else {
  ...
  if (mem_stall) {
    ...
    if (ldAGEX) {
      ...
      for (ii = COPY_AGEX_CS_START; ii< NUM_CONTROL_STORE_BITS; ii++) {
        NEW_PS.AGEX_CS[jj++] = CONTROL_STORE[deADDRESS][ii];
      }
...
void FETCH_stage() {
  ...
  int fetchIR, ldPC, ldDE;
  icache_access(PC, &fetchIR, &icache_r);
  if (v_mem_br_stall) {
    switch(memPCMux) {
      case 0:
        ldPC = 0;
  ...
  } else if (dep_stall | v_de_br_stall | v_agex_br_stall | mem_stall ) {
    ldPC = 0;
  } else {
    if (icache_r) {
      ldPC = 1;
